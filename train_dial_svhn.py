...
from train_utils import train_robust_model, eval_train, eval_test, \
    save_best_robust_model
from whitebox_attack import eval_adv_test_whitebox

parser = argparse.ArgumentParser(description='DIAL SVHN experiment')
...
parser.add_argument('--model-dir', default='dial/model_svhn', help='directory of model for saving checkpoint')
...
parser.add_argument('--domain-loss-ratio', default=2, type=float, help='domain regularization in loss function')
parser.add_argument('--distance', default='linf_kl', help='distance')
parser.add_argument('--schedule', default='svhn', help='learning rate schedule')
...
def main():
    ...
    model = PreActResNetDANN18().to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.test_attack_freq == 0:
            natural_acc, robust_acc = eval_adv_test_whitebox(model, device, test_loader, len(test_loader.dataset),
                                                             args.epsilon, step_size=args.step_size,
            ...
            if robust_acc > best_robust_acc:
                ...
                save_best_robust_model(epoch, model, natural_acc, optimizer, robust_acc,
                                       ...
                                       model_dir=f'{args.model_dir}/best_robust_checkpoint_svhn.pt')
